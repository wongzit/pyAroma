def n1inp(fileName, cpu, mem, nmrType, ruMethod, dft, basis, addKey, charge, spin, coordinate, bqcoor):
	...
	for i in range(len(coordinate)):
		nics1Input.write(f'{coordinate[i][0]}      {coordinate[i][1]}      {coordinate[i][2]}      {coordinate[i][3]}\n')
...
def n2inp(fileName, cpu, mem, nmrType, ruMethod, dft, basis, addKey, charge, spin, coordinate, planeFlag, \
	...
	for i in range(len(coordinate)):
		nicsInput.write(f'{coordinate[i][0]}      {coordinate[i][1]}      {coordinate[i][2]}      {coordinate[i][3]}\n')
...
def n3inp(fileName, cpu, mem, nmrType, ruMethod, dft, basis, addKey, charge, spin, coordinate, \
	...
	for j in range(len(coordinate)):
		nics3Input.write(f'{coordinate[j][0]}      {coordinate[j][1]}      {coordinate[j][2]}      {coordinate[j][3]}\n')
	if fileNumbers == 1:
	...
	if fileNumbers > 1:
		for fileNumber in range(2, fileNumbers + 1):
			...
			for k in range(len(coordinate)):
				nics3Input.write(f'{coordinate[k][0]}      {coordinate[k][1]}      {coordinate[k][2]}      {coordinate[k][3]}\n')
			bqCounter2 = 0
