...
#
#                          by Zhe Wang @iCeMS, updated on 2024-02-06
# ===================================================================
...
class HOMAWindow(QMainWindow):
	def __init__(self):
		...
		homaPara = {
		    ...
		    float(config_file.get('homa', 'no').split(',')[1])],
		    'NS':[float(config_file.get('homa', 'ns').split(',')[0]), \
		    float(config_file.get('homa', 'ns').split(',')[1])],
		    'SN':[float(config_file.get('homa', 'ns').split(',')[0]), \
		    float(config_file.get('homa', 'ns').split(',')[1])],
		    'BB':[float(config_file.get('homa', 'bb').split(',')[0]), \
		    float(config_file.get('homa', 'bb').split(',')[1])],
		    'BC':[float(config_file.get('homa', 'bc').split(',')[0]), \
		    float(config_file.get('homa', 'bc').split(',')[1])],
		    'CB':[float(config_file.get('homa', 'bc').split(',')[0]), \
		    float(config_file.get('homa', 'bc').split(',')[1])],
...
*************************************************************************
'''
class HOMERWindow(QMainWindow):
	def __init__(self):
		super().__init__()
		self.setWindowTitle('HOMER: ' + fileName.split('/')[-1])

		homerMainWidget = QWidget()
		homerMainLayout = QHBoxLayout()

		global molView31
		molView31 = Mpl3DCanvas()
		molView31.ax.axis('off')
		...
		# Load homer from config.ini
		global homerPara
		homerPara = {
		    'CC':[float(config_file.get('homer', 'cc').split(',')[0]), \
		    float(config_file.get('homer', 'cc').split(',')[1])],
		    'CN':[float(config_file.get('homer', 'cn').split(',')[0]), \
		    float(config_file.get('homer', 'cn').split(',')[1])],
		    'NC':[float(config_file.get('homer', 'cn').split(',')[0]), \
		    float(config_file.get('homer', 'cn').split(',')[1])],
		    'CO':[float(config_file.get('homer', 'co').split(',')[0]), \
		    float(config_file.get('homer', 'co').split(',')[1])],
		    'OC':[float(config_file.get('homer', 'co').split(',')[0]), \
		    float(config_file.get('homer', 'co').split(',')[1])],
		    'NN':[float(config_file.get('homer', 'nn').split(',')[0]), \
		    float(config_file.get('homer', 'nn').split(',')[1])]
		}

		if xMax == xMin:
			molView31.ax.set_xlim(xMin - 1, xMin + 1)
			molView31.ax.set_box_aspect([2, yMax - yMin, zMax - zMin])
		elif yMax == yMin:
			molView31.ax.set_ylim(yMin - 1, yMin + 1)
			molView31.ax.set_box_aspect([xMax - xMin, 2, zMax - zMin])
		elif zMax == zMin:
			molView31.ax.set_zlim(zMin - 1, zMin + 1)
			molView31.ax.set_box_aspect([xMax - xMin, yMax - yMin, 2])
		else:
			molView31.ax.set_box_aspect([xMax - xMin, yMax - yMin, zMax - zMin])
		
		for bndAtom_k in bndAtom:
			molView31.ax.plot([xCoor[bndAtom_k[0]], xCoor[bndAtom_k[1]]], [yCoor[bndAtom_k[0]], yCoor[bndAtom_k[1]]], \
				[zCoor[bndAtom_k[0]], zCoor[bndAtom_k[1]]], '0.5') 
		molView31.ax.scatter(np.array([xCoor]), np.array([yCoor]), np.array([zCoor]), color = colorList, edgecolors = '0.0', s = 40, depthshade = 0)

		global monoCycles, centerHOMERList
		monoCycles = geomAnalyzer.find_monocycle(bndAtom)
		centerHOMERList = []  # Save center coordiate and HOMER value of each monocycle
		for monoCycle in monoCycles:
			homerValue = homaCalc.mono_homa(monoCycle, geomList, homerPara)
			if homerValue == -9999999999:
				pass
			else:
				# find center coordinate
				x_center = 0
				...
				for cycleAtom_i in monoCycle:
					...
					y_center += geomList[cycleAtom_i][2]/len(monoCycle)
					z_center += geomList[cycleAtom_i][3]/len(monoCycle)
				centerHOMERList.append([x_center, y_center, z_center, homerValue])

		toolBar31 = NavigationToolbar(molView31, self)

		viewLayout31 = QVBoxLayout()
		viewLayout31.addWidget(molView31)
		viewLayout31.addWidget(toolBar31)
		viewLayout31.setContentsMargins(0, 0, 0, 0)

		viewer31 = QWidget()
		viewer31.setLayout(viewLayout31)

		homerResult = QWidget()
		homerResultLayout = QVBoxLayout()
		homerResult.setLayout(homerResultLayout)

		global homerSum
		homerSum = QTextEdit()
		homerSum.append('Monocycle:\nHOMER Value\n-------------------------------\n')
		for cycle_i in monoCycles:
			ringStr = ''
			for cycle_len in range(len(cycle_i)):
				ringStr = ringStr + geomList[cycle_i[cycle_len]][0] + str(cycle_i[cycle_len] + 1) + '-'
			homerSum.append(f'{ringStr[:-1]}:\n{round(centerHOMERList[monoCycles.index(cycle_i)][3], 5)}\n')

		homerResultLayout.addWidget(homerSum)

		global homerNoText
		homerNoText = QLineEdit()
		homerNoText.setPlaceholderText('Enter atom No. here, e.g.: 1 2 3 4 5 6')
		homerResultLayout.addWidget(homerNoText)

		homerCompBtn = QPushButton('Calculate HOMER for specified ring.')
		homerCompBtn.clicked.connect(self.calUserHOMER)
		homerResultLayout.addWidget(homerCompBtn)

		homerBtnWidget = QWidget()
		homerBtnLayout = QHBoxLayout()
		homerBtnWidget.setLayout(homerBtnLayout)

		self.showHOMERBtn = QCheckBox('Show HOMER Value')
		self.showHOMERBtn.setCheckState(Qt.CheckState.Unchecked)
		self.showHOMERBtn.stateChanged.connect(self.showHOMER)
		homerBtnLayout.addWidget(self.showHOMERBtn)

		self.showLabelBtn = QCheckBox('Show Atom No.')
		self.showLabelBtn.setCheckState(Qt.CheckState.Unchecked)
		self.showLabelBtn.stateChanged.connect(self.showHOMER)
		homerBtnLayout.addWidget(self.showLabelBtn)

		homerResultLayout.addWidget(homerBtnWidget)
		homerResultLayout.setContentsMargins(0, 0, 0, 0)

		homerMainLayout.addWidget(viewer31)
		homerMainLayout.addWidget(homerResult)
		homerMainWidget.setLayout(homerMainLayout)

		self.setCentralWidget(homerMainWidget)

	def showHOMER(self):
		molView31.ax.cla()
		molView31.ax.grid(False)
		molView31.ax.axis('off')
		...
		if self.showHOMERBtn.isChecked():
			if len(centerHOMERList) != 0:
				for homerEle in centerHOMERList:
					molView31.ax.text(homerEle[0], homerEle[1], homerEle[2], round(homerEle[3], 3),\
					bbox = dict(boxstyle = 'round', ec = (0., 0.5, 0.5), fc = (0., 0.8, 0.8)))

		if self.showLabelBtn.isChecked():
			for atom_i in range(len(geomList)):
				molView31.ax.text(geomList[atom_i][1], geomList[atom_i][2], geomList[atom_i][3], atom_i + 1, color = 'darkred')
		
		molView31.draw()
		
	def calUserHOMER(self):
		atmNoStr = str(homerNoText.text())
		if atmNoStr.strip() != '':
			atmNoList = []
			for atom_j in atmNoStr.split():
				atmNoList.append(int(atom_j) - 1)
			if len(atmNoList) >= 3:
				usrHomer = homaCalc.mono_homa(atmNoList, geomList, homerPara)
				if usrHomer == -9999999999:
					QMessageBox.critical(None, 'Unsupported Atom Type', 'Including unsupported atom type.')
				else:
					ringStr2 = ''
					x_cen = 0
					...
					for cycleAtom_j in atmNoList:
						...
						y_cen += geomList[cycleAtom_j][2]/len(atmNoList)
						z_cen += geomList[cycleAtom_j][3]/len(atmNoList)
					centerHOMERList.append([x_cen, y_cen, z_cen, usrHomer])
					homerSum.append(f'{ringStr2[:-1]}:\n{round(usrHomer, 5)}')
			else:
				QMessageBox.critical(None, 'Bad Number of Atoms', 'Please input at least three atoms.')
...
class ScanWindow(QMainWindow):
	def __init__(self):
		super().__init__()
		self.setWindowTitle('NICS-XY-Scan: ' + fileName.split('/')[-1])
		...
		scanParaGroup = QGroupBox('NICS-XY-Scan Parameters')
...
class TypeAWindow(QMainWindow):
	def __init__(self):
		...
		self.homer_window = None
		homerAction = QAction(QIcon('./assets/homer.png'), 'HOMER', self)
		homerAction.setStatusTip('Compute HOMER value for monocycles.')
		homerAction.triggered.connect(self.homerFunction)
		...
		nicsScanAction = QAction(QIcon('./assets/scan.png'), 'NICS-XY-Scan', self)
		nicsScanAction.setStatusTip('Create input file for NICS-XY-scan calculations.')
		...
		window_toolbar.addSeparator()
		window_toolbar.addAction(homerAction)
		window_toolbar.addSeparator()
	...
	def homerFunction(self):
		if self.homer_window is None:
			self.homer_window = HOMERWindow()
		self.homer_window.show()
...
class NICSscanWindow(QMainWindow):
	def __init__(self):
		...
		self.setWindowTitle('NICS-XY-Scan Output: ' + fileName.split('/')[-1])
		global logBqList
		logBqList = atomList[len(geomList) - len(atomList):]
		global xBqCoor, yBqCoor, zBqCoor, colorBqList
		xBqCoor, yBqCoor, zBqCoor, colorBqList = geomAnalyzer.save_coor_list(logBqList)
...
class SetWindow(QMainWindow):
	def __init__(self):
		...
		global ccRopt, cnRopt, coRopt, cpRopt, csRopt, nnRopt, noRopt, nsRopt, bbRopt, bcRopt, bnRopt, cseRopt
		global ccAlpha, cnAlpha, coAlpha, cpAlpha, csAlpha, nnAlpha, noAlpha, nsAlpha, bbAlpha, bcAlpha, bnAlpha, cseAlpha
		ccRopt = QLineEdit()
		...
		nsLabel = QLabel('N-S')								 # HOMA for N-S
		nsLabel.setAlignment(Qt.AlignmentFlag.AlignCenter)
		homaLayout.addWidget(nsLabel, 8, 0)

		nsRopt = QLineEdit()
		nsRopt.setText(configFile.get('homa', 'ns').split(',')[0])
		homaLayout.addWidget(nsRopt, 8, 2)

		nsAlpha = QLineEdit()
		nsAlpha.setText(configFile.get('homa', 'ns').split(',')[1])
		homaLayout.addWidget(nsAlpha, 8, 4)

		bbLabel = QLabel('B-B')								 # HOMA for B-B
		bbLabel.setAlignment(Qt.AlignmentFlag.AlignCenter)
		homaLayout.addWidget(bbLabel, 9, 0)

		bbRopt = QLineEdit()
		bbRopt.setText(configFile.get('homa', 'bb').split(',')[0])
		homaLayout.addWidget(bbRopt, 9, 2)

		bbAlpha = QLineEdit()
		bbAlpha.setText(configFile.get('homa', 'bb').split(',')[1])
		homaLayout.addWidget(bbAlpha, 9, 4)

		bcLabel = QLabel('B-C')								 # HOMA for B-C
		bcLabel.setAlignment(Qt.AlignmentFlag.AlignCenter)
		homaLayout.addWidget(bcLabel, 10, 0)

		bcRopt = QLineEdit()
		bcRopt.setText(configFile.get('homa', 'bc').split(',')[0])
		homaLayout.addWidget(bcRopt, 10, 2)

		bcAlpha = QLineEdit()
		bcAlpha.setText(configFile.get('homa', 'bc').split(',')[1])
		homaLayout.addWidget(bcAlpha, 10, 4)

		bnLabel = QLabel('B-N')								 # HOMA for B-N
		bnLabel.setAlignment(Qt.AlignmentFlag.AlignCenter)
		homaLayout.addWidget(bnLabel, 11, 0)

		bnRopt = QLineEdit()
		bnRopt.setText(configFile.get('homa', 'bn').split(',')[0])
		homaLayout.addWidget(bnRopt, 11, 2)

		bnAlpha = QLineEdit()
		bnAlpha.setText(configFile.get('homa', 'bn').split(',')[1])
		homaLayout.addWidget(bnAlpha, 11, 4)

		cseLabel = QLabel('C-Se')								 # HOMA for C-Se
		cseLabel.setAlignment(Qt.AlignmentFlag.AlignCenter)
		homaLayout.addWidget(cseLabel, 12, 0)

		cseRopt = QLineEdit()
		cseRopt.setText(configFile.get('homa', 'cse').split(',')[0])
		homaLayout.addWidget(cseRopt, 12, 2)

		cseAlpha = QLineEdit()
		cseAlpha.setText(configFile.get('homa', 'cse').split(',')[1])
		homaLayout.addWidget(cseAlpha, 12, 4)

		homaGroup.setLayout(homaLayout)

		homerGroup = QGroupBox('HOMER Parameters')            # HOMER
		mainLayout.addWidget(homerGroup)

		homerLayout = QGridLayout()

		bondTypeLabel = QLabel('Bond Type')
		boldFont = QFont()
		boldFont.setBold(True)
		bondTypeLabel.setFont(boldFont)
		homerLayout.addWidget(bondTypeLabel, 0, 0)

		homerLayout.addWidget(QLabel('      '), 0, 1)

		roptMerLabel = QLabel('Optimal Bond Length (Å)')
		roptMerLabel.setFont(boldFont)
		homerLayout.addWidget(roptMerLabel, 0, 2)

		homerLayout.addWidget(QLabel('      '), 0, 3)

		alphaMerLabel = QLabel('Normalization Constant')
		alphaMerLabel.setFont(boldFont)
		homerLayout.addWidget(alphaMerLabel, 0, 4)

		ccLabelMer = QLabel('C-C')								 # HOMER for C-C
		ccLabelMer.setAlignment(Qt.AlignmentFlag.AlignCenter)
		homerLayout.addWidget(ccLabelMer, 1, 0)

		global ccRoptMer, cnRoptMer, coRoptMer, nnRoptMer
		global ccAlphaMer, cnAlphaMer, coAlphaMer, nnAlphaMer
		ccRoptMer = QLineEdit()
		ccRoptMer.setText(configFile.get('homer', 'cc').split(',')[0])
		homerLayout.addWidget(ccRoptMer, 1, 2)

		ccAlphaMer = QLineEdit()
		ccAlphaMer.setText(configFile.get('homer', 'cc').split(',')[1])
		homerLayout.addWidget(ccAlphaMer, 1, 4)

		cnLabelMer = QLabel('C-N')								 # HOMER for C-N
		cnLabelMer.setAlignment(Qt.AlignmentFlag.AlignCenter)
		homerLayout.addWidget(cnLabelMer, 2, 0)

		cnRoptMer = QLineEdit()
		cnRoptMer.setText(configFile.get('homer', 'cn').split(',')[0])
		homerLayout.addWidget(cnRoptMer, 2, 2)

		cnAlphaMer = QLineEdit()
		cnAlphaMer.setText(configFile.get('homer', 'cn').split(',')[1])
		homerLayout.addWidget(cnAlphaMer, 2, 4)

		coLabelMer = QLabel('C-O')								 # HOMER for C-O
		coLabelMer.setAlignment(Qt.AlignmentFlag.AlignCenter)
		homerLayout.addWidget(coLabelMer, 3, 0)

		coRoptMer = QLineEdit()
		coRoptMer.setText(configFile.get('homer', 'co').split(',')[0])
		homerLayout.addWidget(coRoptMer, 3, 2)

		coAlphaMer = QLineEdit()
		coAlphaMer.setText(configFile.get('homer', 'co').split(',')[1])
		homerLayout.addWidget(coAlphaMer, 3, 4)

		nnLabelMer = QLabel('N-N')								 # HOMER for C-P
		nnLabelMer.setAlignment(Qt.AlignmentFlag.AlignCenter)
		homerLayout.addWidget(nnLabelMer, 4, 0)

		nnRoptMer = QLineEdit()
		nnRoptMer.setText(configFile.get('homer', 'nn').split(',')[0])
		homerLayout.addWidget(nnRoptMer, 4, 2)

		nnAlphaMer = QLineEdit()
		nnAlphaMer.setText(configFile.get('homer', 'nn').split(',')[1])
		homerLayout.addWidget(nnAlphaMer, 4, 4)

		homerGroup.setLayout(homerLayout)
	...
	def goDefault(self):
		...
		nsRopt.setText('1.616')
		nsAlpha.setText('71.7')
		bbRopt.setText('1.5665')
		bbAlpha.setText('244.147')
		bcRopt.setText('1.4235')
		bcAlpha.setText('104.507')
		bnRopt.setText('1.402')
		bnAlpha.setText('72.03')
		cseRopt.setText('1.8217')
		cseAlpha.setText('84.9144')
		ccRoptMer.setText('1.437')
		ccAlphaMer.setText('950.74')
		cnRoptMer.setText('1.390')
		cnAlphaMer.setText('506.43')
		coRoptMer.setText('1.379')
		coAlphaMer.setText('164.96')
		nnRoptMer.setText('1.375')
		nnAlphaMer.setText('187.36')
	...
	def saveSet(self):
		...
		configFile.set('homa', 'ns', f'{nsRopt.text()},{nsAlpha.text()}')
		configFile.set('homa', 'bb', f'{bbRopt.text()},{bbAlpha.text()}')
		configFile.set('homa', 'bc', f'{bcRopt.text()},{bcAlpha.text()}')
		configFile.set('homa', 'bn', f'{bnRopt.text()},{bnAlpha.text()}')
		configFile.set('homa', 'cse', f'{cseRopt.text()},{cseAlpha.text()}')
		configFile.set('homer', 'cc', f'{ccRoptMer.text()},{ccAlphaMer.text()}')
		configFile.set('homer', 'cn', f'{cnRoptMer.text()},{cnAlphaMer.text()}')
		configFile.set('homer', 'co', f'{coRoptMer.text()},{coAlphaMer.text()}')
		configFile.set('homer', 'nn', f'{nnRoptMer.text()},{nnAlphaMer.text()}')
...
class MainWindow(QMainWindow):
	...
	def openManual(self):
		webbrowser.open('https://wongzit.github.io/program/pyaroma/manual_v41.pdf')
